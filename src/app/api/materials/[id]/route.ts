...
export async function DELETE(
  ...
  const [material] = await db.select().from(materials)
    .where(and(eq(materials.id, parsed.data), eq(materials.ownerId, user.id))).limit(1);
  if (!material) return Response.json({ error: "Material not found." }, { status: 404 });

  const paths = [material.blobPathname, material.extractedTextBlobPathname]
    .filter((value): value is string => Boolean(value));
  await del([...new Set(paths)]);
  await db.delete(materials).where(and(eq(materials.id, parsed.data), eq(materials.ownerId, user.id)));
