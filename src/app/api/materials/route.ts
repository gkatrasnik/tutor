...
export async function POST(request: Request) {
  ...
  if (sourceType === "pdf") {
    ...
    if (!parsed.success) return Response.json({ error: "Invalid PDF upload details." }, { status: 400 });

    try {
      const blob = await head(parsed.data.url);
      if (blob.pathname !== parsed.data.pathname) {
        return Response.json({ error: "The uploaded file could not be verified." }, { status: 400 });
      }
      ...
      return Response.json({ error: "The uploaded PDF could not be verified." }, { status: 400 });
    }
  }
  ...
  if (sourceType === "text") {
    ...
    if (!parsed.success) {
      return Response.json({ error: parsed.error.issues[0]?.message ?? "Invalid pasted text." }, { status: 400 });
    }
    ...
    try {
      ...
      return Response.json({ error: "Could not store your text. Check the Blob connection and try again." }, { status: 500 });
    }
  }

  return Response.json({ error: "Choose a supported material type." }, { status: 400 });
}

export async function GET() {
  const user = await requireUser();
  const rows = await db.select().from(materials).where(eq(materials.ownerId, user.id));
