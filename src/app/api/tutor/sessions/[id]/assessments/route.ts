...
export async function GET(request: Request, context: Context) {
  ...
  const offset = z.coerce.number().int().min(0).max(1_000_000).safeParse(new URL(request.url).searchParams.get("offset") ?? 0);
  if (!id.success || !offset.success) return Response.json({ error: "Invalid assessment history request." }, { status: 400 });
  try { return Response.json(await getAssessmentHistory(id.data, user.id, offset.data), { headers: { "Cache-Control": "no-store" } }); }
...
export async function POST(request: Request, context: Context) {
  ...
  if (raw.length > 1000) return Response.json({ error: "Assessment request is too large." }, { status: 413 });
  let body: unknown;
