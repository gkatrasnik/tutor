...
const item: AssessmentSummary = {
  ...
  status: "complete",
  score: 70,
  strengths: ["Explains attention"],
  gaps: ["Needs an example"],
  nextStep: "Apply it to studying",
  error: null,
  createdAt: "2026-08-26T10:00:00.000Z",
};
function render(
  overrides: Partial<Parameters<typeof LessonAssessment>[0]> = {},
) {
  return renderToStaticMarkup(
    createElement(LessonAssessment, {
      sessionId: "session",
      initialHistory: { items: [item], hasMore: false },
...
describe("assessment presentation", () => {
  ...
  it("shows an honest empty state and disables assessment before enough exchanges", () => {
    const html = render({
      initialHistory: { items: [], hasMore: false },
      eligible: false,
