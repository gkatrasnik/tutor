...
export function TutorChat({ sessionId, initialMessages, initiallyReadOnly, initiallyActive }: {
  ...
  useEffect(() => { if (busy) end.current?.scrollIntoView({ block: "nearest" }); }, [answer, busy]);

  async function refresh() {
    const response = await fetch(`/api/tutor/sessions/${sessionId}`, { cache: "no-store" });
    const result = await response.json();
    if (!response.ok) throw new Error(result.error ?? "Could not refresh the conversation.");
  ...
  async function send(text: string) {
    ...
    try {
      const response = await fetch(`/api/tutor/sessions/${sessionId}/messages`, { method: "POST",
        headers: { "content-type": "application/json" }, body: JSON.stringify({ requestId: crypto.randomUUID(), message: text.trim() }) });
      if (!response.ok) {
        const result = await response.json();
        throw new Error(result.error ?? "The tutor could not start.");
      }
      if (!response.body) throw new Error("The response stream is missing.");
      await readTutorStream(response.body, (event) => { if (event.type === "delta") setAnswer((current) => current + event.text); });
    } catch (caught) { setError(caught instanceof Error ? caught.message : "The response was interrupted."); setDraft(text); }
    finally {
      try { await refresh(); } catch { setError("Could not reload the saved conversation. Refresh before resending your question."); }
      setQuestion(""); setAnswer(""); setBusy(false); sending.current = false;
    }
  }
  ...
  return <div className="mt-6 space-y-5">
    ...
    <div className="space-y-4" aria-label="Conversation">
      {messages.map((message) => <Card key={message.id} className={message.role === "user" ? "ml-6 bg-emerald-50" : "mr-6 bg-white"}>
        <CardContent className="p-5"><p className="mb-2 text-xs font-semibold text-stone-500">{message.role === "user" ? "You" : "Tutor"}</p>
          <p className="whitespace-pre-wrap break-words leading-7">{message.content}</p>
          {message.status === "failed" ? <p className="text-sm text-red-700">{message.error}</p> : null}
          {message.status === "pending" ? <p className="text-sm text-stone-500">{active ? "Response is processing. Refresh shortly." : "This response was interrupted. You can send your question again."}</p> : null}
          {message.role === "assistant" && message.status === "complete" && message.sourceCount > 0 ? <SourceSheet sessionId={sessionId} messageId={message.id} count={message.sourceCount} /> : null}
        </CardContent>
      </Card>)}
      {busy ? <><Card className="ml-6 bg-emerald-50"><CardContent className="whitespace-pre-wrap break-words p-5">{question}</CardContent></Card><Card className="mr-6"><CardContent className="p-5"><p className="mb-2 text-xs text-stone-500" role="status">Tutor is responding… Not saved yet.</p><p className="whitespace-pre-wrap break-words leading-7">{answer}</p></CardContent></Card></> : null}
      <div ref={end} />
    </div>
