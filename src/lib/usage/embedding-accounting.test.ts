...
beforeEach(() => {
  vi.resetAllMocks();
  mocks.record.mockImplementation(async ({ run }) =>
    run({ recordMetrics: mocks.observe }),
  );
...
it("accounts separately for every document batch and propagates quota attribution", async () => {
  ...
  for (const [options] of mocks.record.mock.calls)
    expect(options).toMatchObject({
      context,
      feature: "embedding",
      model: "openai/text-embedding-3-small",
    });
  expect(mocks.observe.mock.calls.map(([value]) => value.usage.tokens)).toEqual(
    [50, 1],
  );
});
it("accounts for query embeddings and forwards cancellation", async () => {
  ...
  expect(mocks.observe).toHaveBeenCalledWith(
    expect.objectContaining({ providerMetadata: metadata }),
  );
});
...
it("records administrative embeddings against the source owner and synthetic probes without a learner", async () => {
  ...
  expect(query.mock.calls[1][1]).toEqual(
    expect.arrayContaining(["success", "0.0001", 1]),
  );
  query.mockClear();
  await trackedEmbeddingClient(
    database,
