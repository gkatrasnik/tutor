...
import {
  outlineFixture,
  sourceFixture,
} from "@/lib/courses/fixtures.test-support";
let pg: PGlite;
type Query = {
  sql: string;
  params: unknown[];
...
beforeEach(async () => {
  ...
  await pg.exec(
    "TRUNCATE profiles CASCADE; INSERT INTO profiles(id,email) VALUES ('owner','a@example.test'), ('other','b@example.test')",
  );
...
describe("atomic daily reservations", () => {
  it("permits exactly three concurrent ingestions and preserves independent tutor counters", async () => {
    const results = await Promise.allSettled(
      Array.from({ length: 8 }, () => reserveDailyQuota("owner", "ingestion")),
    );
    expect(results.filter((r) => r.status === "fulfilled")).toHaveLength(3);
    expect(results.filter((r) => r.status === "rejected")).toHaveLength(5);
    await reserveDailyQuota("owner", "tutor");
    expect(
      (await pg.query("SELECT turns, ingestions FROM tutor_daily_usage")).rows,
    ).toEqual([{ turns: 1, ingestions: 3 }]);
    await expect(reserveDailyQuota("other", "ingestion")).resolves.toBeTypeOf(
      "string",
    );
  });
  it("releases only unused reservations exactly once and only for their owner", async () => {
    ...
    expect(
      (await pg.query("SELECT ingestions FROM tutor_daily_usage")).rows[0],
    ).toEqual({ ingestions: 1 });
    await Promise.all([
      releaseUnusedQuota(id, "owner"),
      releaseUnusedQuota(id, "owner"),
    ]);
    expect(
      (await pg.query("SELECT ingestions FROM tutor_daily_usage")).rows[0],
    ).toEqual({ ingestions: 0 });
  ...
  it("keeps started requests charged and never releases against a new UTC day", async () => {
    ...
    expect(
      (await pg.query("SELECT turns FROM tutor_daily_usage")).rows[0],
    ).toEqual({ turns: 1 });
    const unused = await reserveDailyQuota("owner", "ingestion");
    await pg.exec(
      "UPDATE tutor_daily_usage SET day = '2000-01-01'; UPDATE ai_quota_reservations SET day = '2000-01-01'",
    );
    await reserveDailyQuota("owner", "ingestion");
    await releaseUnusedQuota(unused, "owner");
    expect(
      (await pg.query("SELECT ingestions FROM tutor_daily_usage ORDER BY day"))
        .rows,
    ).toEqual([{ ingestions: 0 }, { ingestions: 1 }]);
  });
});
...
describe("rolling AI endpoint limit", () => {
  it("admits exactly five simultaneous requests, isolated by authenticated user", async () => {
    const results = await Promise.allSettled(
      Array.from({ length: 10 }, () => enforceAiRateLimit("owner")),
    );
    expect(results.filter((r) => r.status === "fulfilled")).toHaveLength(5);
    await expect(enforceAiRateLimit("other")).resolves.toBeUndefined();
    await pg.exec(
      "UPDATE ai_rate_limits SET requests = array_fill(now() - interval '61 seconds', ARRAY[5]) WHERE owner_id = 'owner'",
    );
    await expect(enforceAiRateLimit("owner")).resolves.toBeUndefined();
    expect(
      (
        await pg.query(
          "SELECT cardinality(requests) AS count FROM ai_rate_limits WHERE owner_id = 'owner'",
        )
      ).rows[0],
    ).toEqual({ count: 1 });
  });
  it("uses a sliding window rather than resetting at a wall-clock minute boundary", async () => {
    await pg.exec(
      "INSERT INTO ai_rate_limits VALUES ('owner', array_fill(now() - interval '30 seconds', ARRAY[5]))",
    );
    await expect(enforceAiRateLimit("owner")).rejects.toMatchObject({
      status: 429,
    });
  });
  it("keys Firewall checks by server identity and fails closed for missing rules", async () => {
    vi.stubEnv("VERCEL_FIREWALL_RATE_LIMIT_ID", "tutor-ai");
    vi.stubEnv("NODE_ENV", "production");
    vi.stubEnv("NEXT_PUBLIC_APP_URL", "https://tutor.example.test");
    vi.stubEnv("VERCEL_URL", "protected-deployment.vercel.app");
    const request = new Request("https://tutor.example.test", {
      headers: {
        host: "attacker.test",
        "x-user-id": "other",
        cookie: "secret",
    ...
    await expect(enforceAiRateLimit("owner", request)).rejects.toMatchObject({
      status: 503,
    });
    const options = mocks.firewall.mock.calls[0][1];
    expect(options.rateLimitKey).toBe("owner");
    expect([...options.headers.entries()]).toEqual([
...
describe("Gateway usage ledger", () => {
  it("stores actual decimal cost and v7 token details without double-counting cached/reasoning tokens", async () => {
    const value = {
      ...
      providerMetadata: {
        gateway: { cost: "0.000001234567890123", generationId: "gateway-123" },
      },
    };
    await recordGateway({
      context: context(),
      ...
      model: "test/model",
      run: async (recorder) => {
        recorder.recordMetrics(value);
        recorder.markFirstToken();
        return "ok";
      },
    });
    const row = (
      await pg.query<{ latency_ms: number; time_to_first_token_ms: number }>(
    ...
    expect(row).toMatchObject({
      ...
      cached_tokens: 40,
      reasoning_tokens: 5,
      cost_usd: "0.000001234567890123",
      gateway_generation_id: "gateway-123",
    });
    expect(row.latency_ms).toBeGreaterThanOrEqual(0);
    expect(row.time_to_first_token_ms).toBeGreaterThanOrEqual(0);
  });
  it("records failures without leaking prompt text and keeps unknown cost nullable", async () => {
    const cause = Object.assign(new Error("secret text and private Blob URL"), {
      statusCode: 429,
    });
    await expect(
      recordGateway({
  ...
  it("marks a reservation started before provider work and refuses unlogged spend", async () => {
    const ctx = {
      ...context(),
      reservationId: await reserveDailyQuota("owner", "ingestion"),
    };
    await recordGateway({
      context: ctx,
      feature: "embedding",
      ...
      run: async () => {
        expect(
          (await pg.query("SELECT state FROM ai_quota_reservations")).rows[0],
  ...
  it("requires the dedicated Gateway key rather than silently using deployment OIDC", async () => {
    ...
    ).rejects.toThrow("dedicated AI_GATEWAY_API_KEY");
    expect(operation).not.toHaveBeenCalled();
    expect(mocks.query).not.toHaveBeenCalled();
  });
  it("keeps the pending record if finalization fails, without repeating the provider operation", async () => {
    ...
    try {
      ...
      expect(
        (await pg.query("SELECT status FROM ai_usage_events")).rows[0],
  ...
  it("keeps explicit zero separate from missing cost and accepts embedding usage", () => {
    ...
    expect(gatewayCost("0")).toBe("0");
    for (const value of [-1, NaN, Infinity, "-0.2", "not a cost"])
      expect(gatewayCost(value)).toBeNull();
    expect(usageMetrics({ usage: { tokens: 17 } })).toMatchObject({
      inputTokens: 17,
      totalTokens: 17,
      outputTokens: null,
      costUsd: null,
  ...
  it("records both outline attempts and retains Gateway cost on invalid structured output", async () => {
    const response = (
      ...
      providerMetadata: {
        gateway: { cost: "0.00001", generationId: crypto.randomUUID() },
      },
    });
    ...
    const ctx = context();
    expect(await generateCourseOutline(sourceFixture, ctx, model)).toEqual(
      outlineFixture,
    );
    const rows = (
      await pg.query(
        "SELECT status, cost_usd, input_tokens, total_tokens, request_id FROM ai_usage_events ORDER BY created_at",
      )
    ).rows;
    expect(rows).toEqual([
      {
        status: "failure",
        cost_usd: "0.000010000000000000",
        input_tokens: 10,
        total_tokens: 15,
