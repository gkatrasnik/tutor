...
it.each([tutor, assessment, ingestion, outline])(
  "blocks AI routes with a safe 429 before any work",
  async (route) => {
    mocks.limit.mockRejectedValue(new AiLimitError("Too many AI requests"));
    const request = new Request("http://localhost/api/test", {
      method: "POST",
      headers: { "x-user-id": "forged-owner" },
      body: JSON.stringify(
        route === tutor
          ? { requestId: crypto.randomUUID(), message: "Hello" }
    ...
    expect(mocks.limit).toHaveBeenCalledExactlyOnceWith(
      "authenticated-owner",
      request,
    );
...
it.each([tutor, assessment, ingestion, outline])(
  "authenticates before rate-limit or Gateway work",
  async (route) => {
    mocks.user.mockRejectedValue(new Error("Sign in"));
    await expect(
      route(new Request("http://localhost", { method: "POST" }), context),
...
it("does not rate-limit read-only assessment history", async () => {
  ...
  expect((await history(new Request("http://localhost"), context)).status).toBe(
    200,
  );
  expect(mocks.limit).not.toHaveBeenCalled();
});
