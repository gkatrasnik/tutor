...
export async function getTutorSession(sessionId: string, ownerId: string) {
  const [session] = await db
    .select({
      id: tutorSessions.id,
      courseId: tutorSessions.courseId,
      lessonId: tutorSessions.lessonId,
      lessonTitle: tutorSessions.lessonTitle,
      objective: tutorSessions.objective,
      ...
      currentSourceVersion: courses.sourceVersion,
      outlineVersion: courses.outlineVersion,
    })
    .from(tutorSessions)
    .innerJoin(courses, eq(courses.id, tutorSessions.courseId))
    .where(
      and(
  ...
  return {
    ...
    readOnly:
      !session.lessonId ||
      session.courseStatus !== "ready" ||
      session.sourceVersion !== session.currentSourceVersion ||
      session.sourceVersion !== session.outlineVersion,
  };
}
...
export async function startTutorSession(lessonId: string, ownerId: string) {
  ...
  const [existing] = insertedId
    ...
    : await db
        ...
        .where(
          and(
            eq(tutorSessions.lessonId, lessonId),
            eq(tutorSessions.ownerId, ownerId),
          ),
        )
        .limit(1);
  if (!existing)
    throw new TutorError(
      "Lesson unavailable. Open your course and generate an up-to-date outline first.",
      404,
    );
  const session = await getTutorSession(existing.id, ownerId);
  if (session.readOnly)
    throw new TutorError(
      "This outline changed. Open the course to start a current lesson.",
    );
...
export async function getTutorMessages(
  ...
  const rows = await db
    .select({
      ...
      chunkIds: messages.retrievedChunkIds,
    })
    .from(messages)
    .where(
      and(eq(messages.sessionId, sessionId), eq(messages.ownerId, ownerId)),
    )
    .orderBy(desc(messages.ordinal))
    .limit(100);
  return rows.reverse().map(({ chunkIds, ...message }) => ({
    ...message,
...
export async function getMessageSources(
  ...
  const [message] = await db
    .select({ ids: messages.retrievedChunkIds })
    .from(messages)
    .where(
      and(
  ...
  const rows = await db
    ...
    .where(
      and(
        inArray(materialChunks.id, message.ids),
        eq(materialChunks.ownerId, ownerId),
        eq(materials.ownerId, ownerId),
        eq(materials.courseId, session.courseId),
      ),
    );
  return message.ids.map((id, index) => ({
    label: index + 1,
    source: rows.find((row) => row.id === id) ?? null,
  }));
}
...
export async function prepareTutorTurn(
  ...
  if (prior.length) {
    if (prior.find((item) => item.role === "user")?.content !== message)
      throw new TutorError(
    ...
    throw new TutorError(
      "This message is already processing or its attempt failed. Refresh the conversation before sending it again.",
    );
  }
  if (session.readOnly)
    throw new TutorError(
  ...
  const claimed = await db.execute(sql`
    ...
    returning id, role
  `);
  const answer = (claimed.rows as { id: string; role: string }[]).find(
    (row) => row.role === "assistant",
  );
  if (!answer)
    throw new TutorError(
      "A response or assessment is already running, or the course changed. Refresh and try again; interrupted attempts unlock after two minutes.",
    );
  ...
  try {
    const recent = await db
      ...
      .where(
        and(
          ...
          eq(messages.ownerId, ownerId),
          eq(messages.status, "complete"),
          sql`${messages.requestId} <> ${requestId}`,
          inArray(
            messages.requestId,
            db
    ...
    await failTutorTurn(
      turn,
      error instanceof TutorError || error instanceof AiLimitError
        ? error.message
        : "The conversation could not be prepared. Please try again.",
    );
    throw error;
  }
}
...
function turnGuard(turn: PreparedTurn) {
  return and(
    eq(tutorSessions.id, turn.session.id),
    eq(tutorSessions.ownerId, turn.ownerId),
    eq(tutorSessions.activeToken, turn.token),
  );
}
...
export async function failTutorTurn(turn: PreparedTurn, error: string) {
  ...
  await db.batch([
    ...
    db
      ...
      .where(
        and(
          eq(messages.id, turn.messageId),
          eq(messages.ownerId, turn.ownerId),
...
export async function completeTutorTurn(
  turn: PreparedTurn,
  content: string,
  chunkIds: string[],
) {
  const currentCourse = and(
    eq(courses.id, turn.session.courseId),
    eq(courses.ownerId, turn.ownerId),
    eq(courses.status, "ready"),
    eq(courses.sourceVersion, turn.session.sourceVersion),
    eq(courses.outlineVersion, turn.session.sourceVersion),
  );
  const guard = and(
    turnGuard(turn),
    sql`${tutorSessions.lessonId} is not null`,
    exists(db.select({ id: courses.id }).from(courses).where(currentCourse)),
  );
  const saved = await db.batch([
    db.execute(
      sql`select id from ${courses} where ${currentCourse} for update`,
    ),
    db.execute(sql`select id from ${tutorSessions} where ${guard} for update`),
    db
      .update(messages)
