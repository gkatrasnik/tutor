...
export async function readTutorStream(body: ReadableStream<Uint8Array>, onEvent: (event: TutorEvent) => void) {
  ...
  try {
    ...
    if (!done || buffer.trim()) throw new Error("The connection ended before saving was confirmed. Refresh the conversation before resending.");
