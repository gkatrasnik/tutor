...
beforeEach(async () => {
  ...
  await pg.query(
    "INSERT INTO courses (id, owner_id, name) VALUES ($1, $2, $3)",
    [courseId, ownerId, sourceFixture.courseName],
  );
  for (const material of sourceFixture.materials) {
    await pg.query(
      ...
      VALUES ($1::uuid, $2, $3, 'text', $4, 'https://example.test/notes', $1::text, 100, 'ready')`,
      [material.id, courseId, ownerId, material.filename],
    );
    for (const chunk of material.chunks) {
      await pg.query(
        `INSERT INTO material_chunks (material_id, owner_id, ordinal, page_number, content, token_count, embedding)
        VALUES ($1, $2, $3, $4, $5, 10, '{1,2,3}')`,
        [material.id, ownerId, chunk.ordinal, chunk.pageNumber, chunk.content],
      );
    }
  }
...
async function storedLessons() {
  return (
    await pg.query<{ title: string; ordinal: number }>(
      "SELECT title, ordinal FROM lessons WHERE course_id = $1 ORDER BY ordinal",
      [courseId],
    )
  ).rows;
}
...
describe("course-wide generation with Neon HTTP SQL executed in PostgreSQL", () => {
  it("combines both materials and atomically saves ordered lessons", async () => {
    expect(await ensureCourseOutline(courseId, ownerId)).toEqual({
      id: courseId,
      status: "ready",
    });
    ...
    expect(await storedLessons()).toEqual(
      outlineFixture.lessons.map((lesson, ordinal) => ({
        title: lesson.title,
        ordinal,
      })),
    );
  ...
  it("rejects another owner before reading sources or generating", async () => {
    await expect(
      ensureCourseOutline(courseId, "learner-b"),
  ...
  it("does not include another course's sources, even with the same owner", async () => {
    ...
    await pg.query(
      "INSERT INTO courses (id, owner_id, name) VALUES ($1, $2, 'Other')",
      [other, ownerId],
    );
    await pg.query("UPDATE materials SET course_id = $1 WHERE id = $2", [
      other,
      sourceFixture.materials[1].id,
    ]);
    await ensureCourseOutline(courseId, ownerId);
    expect(mocks.generate.mock.calls[0][0].materials).toEqual([
      sourceFixture.materials[0],
    ]);
  });

  it.each(["uploaded", "processing", "failed"])(
    "blocks generation when any material is %s",
    async (status) => {
      await pg.query("UPDATE materials SET status = $1 WHERE id = $2", [
        status,
        sourceFixture.materials[1].id,
      ]);
      await expect(
        ensureCourseOutline(courseId, ownerId),
  ...
  it("rejects oversized combined sources instead of silently truncating", async () => {
    await pg.query("UPDATE material_chunks SET content = $1", [
      "x".repeat(100_001),
    ]);
    await expect(ensureCourseOutline(courseId, ownerId)).rejects.toMatchObject({
      status: 422,
    });
    expect(mocks.generate).not.toHaveBeenCalled();
  });
  ...
  it("keeps the course and previous outline when a material is deleted, then replaces lessons on update", async () => {
    await ensureCourseOutline(courseId, ownerId);
    await pg.query("DELETE FROM materials WHERE id = $1", [
      sourceFixture.materials[1].id,
    ]);
    expect(await course()).toMatchObject({
      source_version: 3,
    ...
    mocks.generate.mockResolvedValue({
      ...outlineFixture,
      lessons: outlineFixture.lessons.slice().reverse(),
    });
    await ensureCourseOutline(courseId, ownerId);
    expect(await course()).toMatchObject({
      source_version: 3,
  ...
  it("allows retrying an expired claim and fences the stale worker", async () => {
    ...
    await startedPromise;
    await pg.exec(
      "UPDATE courses SET generation_started_at = now() - interval '6 minutes'",
    );
    mocks.generate.mockResolvedValue({
      ...outlineFixture,
      title: "New outline",
