...
async function assertCompatibleIndex(scope: SQL | undefined) {
  const [incompatible] = await db.select({ id: materialChunks.id }).from(materialChunks)
    .innerJoin(materials, eq(materials.id, materialChunks.materialId))
    .innerJoin(courses, eq(courses.id, materials.courseId))
    .where(and(scope, sql`${materialChunks.embeddingModel} is distinct from ${env.EMBEDDING_MODEL}`)).limit(1);
  if (incompatible) throw new EmbeddingError("These materials need an embedding migration before retrieval. Run the embedding-only re-embedding command for the configured model.");
}
