...
describe("material chunking", () => {
  it("creates ordered, overlapping chunks near the target size", () => {
    const chunks = chunkMaterialPages([
      { pageNumber: null, text: words(1_000) },
    ]);
    ...
    const sharedTailTokens = firstWords
      .slice(-CHUNK_OVERLAP_TOKENS)
  ...
  it("keeps PDF page metadata on every chunk", () => {
    ...
    expect(
      chunks
        ...
        .every((chunk) => chunk.content.includes("first")),
    ).toBe(true);
    expect(
      chunks
        .filter((chunk) => chunk.pageNumber === 2)
        .every((chunk) => chunk.content.includes("second")),
    ).toBe(true);
  });

  it("rejects materials above the chunk cap", () => {
    const pages = Array.from(
      { length: MAX_CHUNKS_PER_MATERIAL + 1 },
      (_, index) => ({
        pageNumber: index + 1,
        text: `Page ${index + 1}`,
      }),
