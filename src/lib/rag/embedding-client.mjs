...
export function validateEmbedding(embedding, dimensions) {
  if (
    ...
    throw new EmbeddingError(
      `The embedding provider returned an invalid vector. Expected ${dimensions} float values.`,
    );
  }
  return embedding;
}
...
export function embeddingProviderOptions(model, dimensions, inputType) {
  ...
  throw new EmbeddingError(
    `Unsupported embedding model: ${model}. Configure and migrate a supported embedding model first.`,
  );
}
...
function embeddingFailure(error, operation) {
  ...
  return new EmbeddingError(
    isRateLimit(error)
      ...
      : `Tutor could not ${operation} through AI Gateway. Check the Gateway connection and retry.`,
    { cause: error },
  );
}
  ...
  return {
    ...
    async embedDocuments(values, signal) {
      ...
      try {
        ...
        for (
          ...
          embeddings.push(
            ...result.embeddings.map((value) =>
              validateEmbedding(value, dimensions),
            ),
          );
        }
      } catch (error) {
        throw embeddingFailure(error, "index this material");
      }
      return embeddings;
    },
    ...
    async embedQuery(value, signal) {
      try {
        ...
        throw embeddingFailure(error, "run retrieval");
      }
    },
  };
}
