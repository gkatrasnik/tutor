...
vi.mock("server-only", () => ({}));
vi.mock("./embeddings", () => ({ embedQuery: mocks.embed, EmbeddingError: class extends Error {} }));
...
describe("course-wide retrieval SQL", () => {
  ...
  it("rejects unknown or incompatible vectors before any embedding request", async () => {
    ...
    await expect(retrieveCourseChunks({ ownerId: "learner-a", courseId: "course-a", query: "test" })).rejects.toThrow("embedding migration");
  ...
  it("also guards and filters material-level retrieval", async () => {
    await retrieveMaterialChunks({ ownerId: "learner-a", materialId: "material-a", query: "test" });
    expect(mocks.query.mock.calls[0][1]).toEqual(expect.arrayContaining(["learner-a", "material-a", "openai/text-embedding-3-small"]));
    expect(mocks.query.mock.calls[1][0]).toContain('"material_chunks"."embedding_model" =');
  });
});
