...
One chat input accepts both answers and help requests. The tutor infers intent from the current part and conversation: clear answer attempts receive brief, unscored feedback; help requests, mixed answer/help messages, and uncertain intent stay on the current part. Feedback is buffered and checked for question marks before publication, with one regeneration attempt if needed. The prompt also prohibits questions and next-part content in feedback. These model-based checks reduce mistakes but cannot guarantee teaching quality or intent accuracy.

After answer feedback, **Continue** reveals the next saved explanation and question without an AI call or daily tutor-turn charge. Learners can still ask for help before continuing. The saved plan, passage order, answered-part count, and waiting-for-Continue state persist across navigation and reloads; a failed tutor response does not advance progress. After the final answer, **Test** becomes available directly. Older saved plans keep their content and resume from the part already displayed; newly generated plans receive the new coverage checks.
