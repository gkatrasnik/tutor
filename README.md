...
Starting a lesson creates or resumes a persistent session. The tutor prepares 3–6 small explanation-and-question parts from retrieved course passages. Before a new plan is shown, structure checks and a separate model review check that each part teaches one concept and supplies the knowledge needed for its single question. Invalid plans get one regeneration attempt.

One chat input accepts both answers and help requests. Every clear answer attempt receives brief feedback and automatically reveals the next prepared explanation and question, regardless of correctness. Incorrect answers receive a brief correction without another attempt. Help requests, mixed answer/help messages, and uncertain intent stay on the current part; after the explanation, the application repeats the exact saved question so the learner knows what to answer. Feedback streams incrementally from the model into the chat. The preview is provisional: completed feedback is checked for question marks, with one regeneration attempt if needed; a retry clears the previous preview and a failure clears unsaved text. Lesson progress changes only after validation and successful persistence. The application appends questions and lesson transitions after saving. Model checks reduce mistakes but cannot guarantee teaching quality or prevent all invalid text appearing briefly during generation. Initial lesson plans still complete their independent review before publication.

There is no manual **Continue** button between lesson parts. Automatic advancement reuses the saved explanation and question without another AI call or daily tutor-turn charge. The saved plan, passage order, and completed-part count persist across navigation and reloads. Replaying a request does not advance twice. After the learner answers the final part, regardless of correctness, Tutor explicitly says that all parts are finished and the test can be started. Older sessions paused at Continue automatically reveal their next saved part when opened; legacy incorrect-attempt counts no longer affect progression.
