...
import { createEmbeddingClient, DEFAULT_EMBEDDING_MODEL, EmbeddingError } from "../src/lib/rag/embedding-client.mjs";
...
async function main() {
  ...
  const client = createEmbeddingClient({ model, dimensions });
  ...
  await probe();
  const updated = await reembedIndex({ database, model, dimensions, index, client,
...
main().catch((error) => {
  ...
  else if (error?.code === "42703") console.error("Missing embedding model column. Run pnpm db:migrate first.");
